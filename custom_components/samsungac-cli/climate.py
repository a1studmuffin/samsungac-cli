...
DEFAULT_SMARTTHINGS_PATH = "/root/smartthings/smartthings"
...
PLATFORM_SCHEMA = PLATFORM_SCHEMA.extend(
    {
        ...
        voluptuous.Required(CONF_SMARTTHINGS_PATH, default=DEFAULT_SMARTTHINGS_PATH): cv.string,
    }
)
...
class SamsungACCLIClimate(ClimateEntity):
    ...
    @property
    def hvac_modes(self):
        return [HVAC_MODE_AUTO, HVAC_MODE_COOL, HVAC_MODE_DRY, HVAC_MODE_FAN_ONLY, HVAC_MODE_HEAT, HVAC_MODE_OFF]
    ...
    def run_smartthings_command(self, command):
        try:
            ...
            result = subprocess.run(smartthings_command.split(), capture_output=True, text=True)
    ...
    async def async_update(self):
        ...
        try:
            ...
            self._min_temp = device_status_json["components"]["main"]["custom.thermostatSetpointControl"]["minimumSetpoint"]["value"] # just assume celcius here
            self._max_temp = device_status_json["components"]["main"]["custom.thermostatSetpointControl"]["maximumSetpoint"]["value"] # just assume celcius here
            
            self._current_mode = device_status_json["components"]["main"]["airConditionerMode"]["airConditionerMode"]["value"]
            self._switch_state = device_status_json["components"]["main"]["switch"]["switch"]["value"]
            ...
            self._current_fan_mode = device_status_json["components"]["main"]["airConditionerFanMode"]["mode"]["value"]
            ...
            self._current_temp = device_status_json["components"]["main"]["temperatureMeasurement"]["temperature"]["value"] # just assume celcius here
            self._target_temp = device_status_json["components"]["main"]["thermostatCoolingSetpoint"]["coolingSetpoint"]["value"] # just assume celcius here
            self._power_consumption = device_status_json["components"]["main"]["powerConsumptionReport"]["powerConsumption"]["power"] # just assume celcius here

            _LOGGER.debug(f'State update complete, Target temp: {self._target_temp}; Current temp: {self._current_temp}; Min temp: {self._min_temp}; Max temp: {self._max_temp}; Current mode: {self._current_mode}; Switch state: {self._switch_state}; Current fan mode: {self._current_fan_mode}; Power consumption: {self._power_consumption};')

        except Exception as e:
            _LOGGER.debug(f'State update failed to parse json, setting to unavailable: {str(e)}')
            self._available = False
    ...
    async def async_set_hvac_mode(self, **kwargs) -> None
        ...
        if self._switch_state == 'on':
            ...
            self.run_smartthings_command(f'airConditionerMode:setAirConditionerMode({self._current_mode})')
